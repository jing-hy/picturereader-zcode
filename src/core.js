 ...
 * tool execution (see `importCore` in mcp/server.js), so edits to this file take
 * effect on the next `image_scan` call WITHOUT restarting the MCP server. That
 * only works because this module has no relative imports of its own source
 * files: the only static dependencies are stable npm packages (pngjs /
 * jpeg-js / omggif), which Node keeps cached.
 *
...
/**
 * Full OCR pipeline: decode -> optional region crop -> (paddle only: optional
 * downscale to the input cap) -> PNG temp file -> OCR engine -> cleanup.
 * Returns recognized text lines with pixel boxes.
 ...
 * @returns `{ width, height, lines, downscaled? }` where width/height are the
 *   dimensions actually fed to the engine (downscaled for paddle when the
...
export async function ocrImage(buffer, ext, { region, language, engine = 'windows' } = {}) {
  ...
  if (engine === 'paddle') {
    const resized = downscaleRgba(work.data, work.width, work.height);
    if (resized.downscaled) {
      work = { data: resized.data, width: resized.width, height: resized.height };
      downscaled = true;
    }
  }
  ...
  try {
    if (engine === 'paddle') {
      ...
      return { width: work.width, height: work.height, lines: result.lines, ...(downscaled ? { downscaled } : {}) };
    }
...
/**
 * PaddleX model cache, defaulting to the plugin's own `.paddlex-cache` dir;
 * overridable via DSH_PADDLE_CACHE.
 */
export function paddleCacheHome() {
  if (process.env.DSH_PADDLE_CACHE) return process.env.DSH_PADDLE_CACHE;
  return fileURLToPath(new URL('../.paddlex-cache/', import.meta.url));
}
